...
- Codex 里用 `/goal` 开的会话不再显示空标题: 该方式设置的目标不经过 hook, 现在会在每轮结束时从会话记录中取回, 作为这个 session 的人类诉求展示 (重复目标只记一条, 中途改目标会新增一条)。

## [0.24.0] - 2026-07-30

### Changed

- 采集与展示全面放开, 不再为性能保留上限: 单条事件额度由 6 万字符提到 1.4 MB (整段工具输出 / 超长 prompt 原样入库), 数组不再截条数, session 时间线一次返回全部事件, session 列表可取全部, 卡片提示词预览 300 -> 1000 字符。
- 事件细节展示补全: 未识别 / 新增事件的摘要行自动列出自身关键字段 (原先空白), 批量工具调用 / 压缩摘要 / 斜杠命令展开 / 通知类型等补进摘要; 工具调用展开后新增可读的输出正文 (原先只有转义过的原始 JSON), 失败信息可点开全文, subagent 最终回复改为大块展示。
- 内容被宿主 agent 自己截断时 (标记 `[truncated, N chars total]`, 通常是每轮注入的提示文本) 挂 `host-truncated` 标记: 原文没交给 hook, 本工具无法还原, 明示而非让人以为是页面隐藏了数据。

### Fixed

- 上报不再有静默丢弃: 无法解析的 hook 数据原样记录成一条事件, 单条超额时保留全部字段名与占位说明并标 `truncated`, 上传失败自动重试一次。
- 长 session 的时间线原先只返回最早的一段 (而非最新), 现返回全部; assistant 过程文本被工具调用打断后不再拆成多块; 与最终回复重复的过程文本改为折叠一行而非删除, 点开即完整原文。
...
- session 时间线充分利用宽屏空间, 长命令 / 日志减少无效换行。
